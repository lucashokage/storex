...
export default function NovedadesPage() {
  ...
  // Obtener los productos más recientes (simulado por ID más alto)
  const novedades = [...products].sort((a, b) => b.id - a.id).slice(0, 6)
  ...
  return (
    <main className="min-h-screen">
      ...
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-8 text-[#9D2449]">Novedades</h1>

        {novedades.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-muted-foreground mb-8">No hay productos nuevos disponibles</p>
            <Link href="/">
              ...
              <Link href={`/product/${product.id}`} key={product.id} className="group">
                <div className="bg-[#FFF5F5] rounded-lg overflow-hidden">
                  <div className="aspect-square relative">
                    ...
                    <div className="absolute top-2 left-2 bg-[#9D2449] text-white px-2 py-1 rounded-md text-sm font-bold">
                      NUEVO
                    </div>
                  </div>
                </div>
